# -*- coding: utf-8 -*-
"""
Created on Fri Apr 10 03:00:18 2020
...
def region_growth(filename, x, y, destination, threshold=1, conn=4):
...
def draw_circle(event, x, y, flags, params):
    if event == cv2.EVENT_LBUTTONDBLCLK:
        cv2.circle(img21, (x, y), 3, (255, 0, 0), -1)
        mouseX1, mouseY1 = x, y
        fla1 = False

def getInput(x12):
    img21 = cv2.imread(x12, 1)
    winname = 'Double Click to Select'
    cv2.namedWindow(winname)
    cv2.setMouseCallback(winname, draw_circle)                
    while 1:
        cv2.imshow(winname, img21)
        ...
        if fla1==False:
            cv2.destroyAllWindows()
            break
    return str(mouseX1), str(mouseY1)
...
class Window(Frame):
    ...
    def mouse_selection(self):
        self.x2, self.y2 = getInput(self.file_name)
    ...
    def submit(self):
        if self.file_name == None:
            ...
            if self.var1.get() == '1':
                reg2 = region_growth(self.file_name, int(self.y1.get()), int(self.x1.get()), self.dest_name, int(self.tentry.get()), int(self.conn.get()))
            elif self.var1.get() == '2':
                reg2 = region_growth(self.file_name, int(self.y2), int(self.x2), self.dest_name, int(self.tentry.get()), int(self.conn.get()))
